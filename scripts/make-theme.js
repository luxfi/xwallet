...
import { themeColors, rabbyCssPrefix } from '../src/constant/theme-colors';

const ROOT = resolve(__dirname, '..');

const cssvarSrcfile = resolve(ROOT, 'src/ui/style/cssvars.css');
...
${
  ...
  .join(LINE_BREAK)}
`;
  writeFileSync(cssvarSrcfile, cssvarSrcContent, 'utf8');

  console.log('[rabby] make-theme css vars version success!');
}

{
  const varsLess = readFileSync(
    resolve(ROOT, './src/ui/style/var.less'),
    'utf8'
  );
  ...
  const fname = basename(__filename);

  writeFileSync(
    resolve(ROOT, './src/ui/style/var-defs.ts'),
    `\
/* eslint-disable */
