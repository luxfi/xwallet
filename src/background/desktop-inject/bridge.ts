...
async function onLuxxRpcQuery(payload: BridgePayload) {
  if (!payload.rpcId) {
    throw new Error('[luxx-rpc-query] rpcId is required');
  }
  ...
  switch (payload.method) {
    ...
    case 'walletController.unlock': {
      retPayload = await runAndCatchErr(() => {
        ...
        return walletController.unlock.apply(
          walletController,
    ...
    case 'walletController.importPrivateKey': {
      retPayload = await runAndCatchErr(() => {
        const [password] = payload.params || [];
        return walletController.importPrivateKey.apply(walletController, [
          password,
        ]);
      }, payload.method);
      break;
    ...
    case 'walletController.updateAlianName': {
      retPayload = await runAndCatchErr(() => {
        ...
        return walletController.updateAlianName.apply(walletController, [
          address,
          name,
        ]);
      }, payload.method);
    ...
    default: {
      ...
      if (
        ns === 'walletController' &&
        typeof walletController[method] === 'function'
      ) {
        retPayload = await runAndCatchErr(() => {
          // eslint-disable-next-line prefer-spread
          return walletController[method].apply(
            walletController,
            payload.params
          );
        }, payload.method);
      } else if (
        ns === 'permissionService' &&
        typeof permissionService[method] === 'function'
      ) {
        retPayload = await runAndCatchErr(() => {
          // eslint-disable-next-line prefer-spread
          return permissionService[method].apply(
            permissionService,
            payload.params
          );
        }, payload.method);
        ...
        ns === 'openapi' &&
        typeof openapiService[method] === 'function'
      ) {
        retPayload = await runAndCatchErr(() => {
          // eslint-disable-next-line prefer-spread
        ...
        ns === 'testnetOpenapi' &&
        typeof testnetOpenapiService[method] === 'function'
      ) {
        retPayload = await runAndCatchErr(() => {
          // eslint-disable-next-line prefer-spread
          return testnetOpenapiService[method].apply(
        ...
        retPayload.error = {
          message: `[luxx-rpc-query] method ${payload.method} is not supported`,
        };
      }
    }
  }
  ...
  window.luxDesktop.ipcRenderer.sendMessage(
    'luxx-rpc-respond',
    JSON.stringify({
      rpcId: payload.rpcId,
...
if (window.luxDesktop?.ipcRenderer.on) {
  console.warn(
    '[debug] window.luxDesktop?.ipcRenderer.on',
    window.luxDesktop?.ipcRenderer.on
  );
  window.luxDesktop?.ipcRenderer.on('luxx-rpc-query', onLuxxRpcQuery);
} else {
  document.addEventListener('luxx-rpc-query', (e: any) => {
    onLuxxRpcQuery(e.detail);
  });
}
